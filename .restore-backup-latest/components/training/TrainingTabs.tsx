...
import {
  countActiveDrillFilters,
  type DrillCatalogFilters,
} from "@/lib/drill-catalog-filters";
import { METRIC_LABELS, METRICS_BY_CATEGORY, syncShootingTargetStrings } from "@/lib/workout-metrics";
  ...
  return (
    <form id="new-workout-form" className="space-y-3" onSubmit={onCreateWorkout}>
      ...
      <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--bg-muted)] p-3">
        {newWorkoutCategory === "Basketball" && newWorkoutSubcategory === "Spiel" ? null : createWorkoutExerciseOptions.length === 0 ? (
      ...
      {selectedExercises.length > 0 ? (
        <div className="app-card--flat">
          ...
          <GradientFadeList
            ...
            listClassName="list-inside list-disc space-y-1 text-sm text-strong"
            getKey={(exercise) => `selected-${exercise.id}`}
            renderItem={(exercise) => <div>{exercise.name}</div>}
          />
        </div>
      ) : null}
    </form>
  ...
  return (
    <section className="space-y-4">
      ...
      <section className="ui-card">
        ...
        <div className="mt-4">
          {workouts.length === 0 ? (
            <EmptyState title="Noch kein Workout" />
          ) : (
            <GradientFadeList
              items={workouts}
              listClassName="space-y-2"
      ...
      <Modal open={Boolean(editingWorkoutId)} onClose={onCancelEditWorkout} title="Workout bearbeiten">
        <form className="space-y-3" onSubmit={onUpdateWorkout}>
          <FilterSection
            title="Kategorie"
            options={categories}
  ...
  return (
    <>
      ...
      {metrics.length > 0 ? (
        <div className="grid gap-2 sm:grid-cols-2">
          {metrics.map((metric) => {
            const targetValues = isEdit ? editExerciseTargets : newExerciseTargets;
            const syncedTargets = syncShootingTargetStrings(targetValues ?? {});
            const isAutoMisses = metric === "misses" && metrics.includes("reps") && metrics.includes("makes");
            return (
              <input
                key={metric}
                type="number"
                value={isAutoMisses ? syncedTargets.misses ?? "" : targetValues?.[metric] ?? ""}
                readOnly={isAutoMisses}
                onChange={(event) => {
                  if (isAutoMisses) return;
                  if (isEdit) onEditExerciseTargetChange?.(metric, event.target.value);
                  else onNewExerciseTargetChange(metric, event.target.value);
                }}
                placeholder={isAutoMisses ? `${METRIC_LABELS[metric]} (auto)` : `Ziel ${METRIC_LABELS[metric]}`}
                className={`input ${isAutoMisses ? "opacity-80" : ""}`}
                title={isAutoMisses ? "Wird automatisch aus Reps − Makes berechnet" : undefined}
              />
            );
          })}
        </div>
            ...
            {setTargets.map((setTargetRow, setIndex) => (
              <div key={`set-goal-${setIndex}`} className="w-full rounded-lg border border-[var(--surface-border)] p-2">
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  {metrics.map((metric) => {
                    const syncedRow = syncShootingTargetStrings(setTargetRow);
                    const isAutoMisses = metric === "misses" && metrics.includes("reps") && metrics.includes("makes");
                    return (
                    <input
                      key={`set-${setIndex}-${metric}`}
                      type="number"
                      value={isAutoMisses ? syncedRow.misses ?? "" : setTargetRow[metric] ?? ""}
                      readOnly={isAutoMisses}
                      onChange={(event) => {
                        if (isAutoMisses) return;
                        if (isEdit) onEditExerciseSetTargetChange?.(setIndex, metric, event.target.value);
                        else onNewExerciseSetTargetChange(setIndex, metric, event.target.value);
                      }}
                      placeholder={
                        isAutoMisses
                          ? `${METRIC_LABELS[metric]} (auto)`
                          : `${METRIC_LABELS[metric]} (Satz ${setIndex + 1})`
                      }
                      className={`input ${isAutoMisses ? "opacity-80" : ""}`}
                      title={isAutoMisses ? "Wird automatisch aus Reps − Makes berechnet" : undefined}
                    />
...
export function ExercisesTab({
  ...
  return (
    <section className="space-y-4">
      ...
      <DrillCatalogFilterCard filters={drillFilters} onChange={onDrillFilterChange} onReset={onDrillFiltersReset} />

      <section className="ui-card">
        <h2 className="ui-card__title">Exercises</h2>

        <div className="mt-4">
          {visibleExercises.length === 0 ? (
            <EmptyState title="Keine Exercises" />
          ) : (
            <GradientFadeList
      ...
      <Modal open={Boolean(editingExerciseId)} onClose={onCancelEditExercise} title="Exercise bearbeiten">
        <form className="space-y-3" onSubmit={onUpdateExercise}>
          <ExerciseFormFields
...
function DrillCatalogFilterCard({
  ...
  const activeCount = countActiveDrillFilters(filters);
    ...
    <section className="ui-card">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <FilterClearButton onClick={onReset} disabled={activeCount === 0} />
          <button
            type="button"
            onClick={() => setOpen((current) => !current)}
            className="btn btn-ghost btn-sm shrink-0"
            aria-expanded={open}
            aria-label={open ? "Erweiterte Filter schließen" : "Erweiterte Filter öffnen"}
          >
            {open ? "Schließen" : activeCount > 0 ? `Filter (${activeCount})` : "Filter"}
          </button>
        </div>
      </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {filters.video !== "all" ? (
            <span className="filter-chip filter-chip--active">
              Video: {filters.video === "with" ? "Mit Video" : "Ohne Video"}
            </span>
          ) : null}
          {filters.duration !== "all" ? (
            <span className="filter-chip filter-chip--active">
              Dauer: {filters.duration === "under10" ? "Unter 10 Min" : "Unter 15 Min"}
            </span>
          ) : null}
          {filters.equipment !== "all" ? (
            <span className="filter-chip filter-chip--active">
              Ort:{" "}
              {filters.equipment === "gym"
                ? "Gym"
                : filters.equipment === "home"
                  ? "Heim"
                  : "Outdoor (BB)"}
            </span>
          ) : null}
    ...
    <div className="segmented">
      {(["Workouts", "Exercises"] as TrainingTab[]).map((tab) => (
        <button
          ...
          className={`segmented__btn ${activeTab === tab ? "segmented__btn--active" : ""}`}
