...
import { DEFAULT_DRILL_FILTERS, matchesDrillCatalogFilters, type DrillCatalogFilters } from "@/lib/drill-catalog-filters";
...
import { normalizeMetricKeysForCategory, syncShootingTargetNumbers, syncShootingTargetStrings } from "@/lib/workout-metrics";
...
function TrainingPageContent() {
  ...
  const catalogSearchExercises = useMemo(() => {
    return rankByFuzzySearch(
      exercises.filter((exercise) => exercise.subcategory !== "Komplett" && matchesDrillCatalogFilters(exercise, drillFilters)),
      catalogSearch,
  ...
  async function handleAddExercise(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const validationError = validateMetricTargets(
      newExerciseCategory,
      normalizedMetrics,
      syncShootingTargetStrings(newExerciseTargets),
    );
    if (validationError) {
    ...
    const nextExercises = [
      ...exercises,
      {
        ...
        targetByMetric: syncShootingTargetNumbers(
          Object.fromEntries(
            Object.entries(syncShootingTargetStrings(newExerciseTargets)).flatMap(([metric, value]) => {
        ...
        setTargetsByMetric: normalizeSetTargetsLength(newExerciseSetTargets, newExerciseSetCount).map((setTargets) =>
          syncShootingTargetNumbers(
            Object.fromEntries(
              Object.entries(syncShootingTargetStrings(setTargets)).flatMap(([metric, value]) => {
                const parsed = parseMetricInput(value);
                return parsed === null ? [] : [[metric, parsed]];
              }),
            ) as Partial<Record<MetricKey, number>>,
    ...
    setEditExerciseSetTargets(
      normalizeSetTargetsLength(
        (exercise.setTargetsByMetric ?? []).map((setTargets) =>
          syncShootingTargetStrings(
            Object.fromEntries(Object.entries(setTargets ?? {}).map(([metric, value]) => [metric, String(value)])) as Partial<
  ...
  async function handleUpdateExercise(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const validationError = validateMetricTargets(
      ...
      editExerciseMetrics,
      syncShootingTargetStrings(editExerciseTargets),
    );
    if (validationError) {
    ...
    const numericTargets = syncShootingTargetNumbers(
      Object.fromEntries(
        Object.entries(syncShootingTargetStrings(editExerciseTargets)).flatMap(([metric, value]) => {
          const parsed = parseMetricInput(value);
          return parsed === null ? [] : [[metric, parsed]];
        }),
      ) as Partial<Record<MetricKey, number>>,
  ...
  if (!clientReady || !activeTab) {
    return <main className="app-container">Bibliothek wird geladen…</main>;
  }
      ...
      <div className="flex w-full flex-col gap-4">
        <div className="training-top">
          <div className="training-top__main">
            ...
            <TopSubTabs
              items={[
                { label: "Weekly", href: WEEKLY_WORKOUT_PATH },
                { label: "Training", href: buildTrainingHref(activeTab) },
              ]}
            />
          ...
          <div className="training-top__rail">
            ...
            <Link href="/game-track?context=game" className="btn btn-outline btn-xs shrink-0">
              Spieltag starten
            </Link>
            <Link href="/game-track?context=game_training" className="btn btn-outline btn-xs shrink-0">
        ...
        {completionMessage ? (
          <div className="flex flex-wrap items-center justify-between gap-2 app-card--accent-emerald">
        ...
        {activeTab === "Workouts" ? (
          ...
          <ExercisesTab
            ...
            onNewExerciseTargetChange={(metric, value) =>
              setNewExerciseTargets((current) => syncShootingTargetStrings({ ...current, [metric]: value }))
            }
            ...
            onNewExerciseSetTargetChange={(setIndex, metric, value) =>
              setNewExerciseSetTargets((current) => {
                ...
                normalized[setIndex] = syncShootingTargetStrings({ ...(normalized[setIndex] ?? {}), [metric]: value });
                return [...normalized];
              })
            }
            ...
            onEditExerciseTargetChange={(metric, value) =>
              setEditExerciseTargets((current) => syncShootingTargetStrings({ ...current, [metric]: value }))
            }
